...
class PlayState extends MusicBeatState
{
	...
	//splatoon ezili character variables
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
					halloweenBG = new BGSprite('halloween_bg', 0, -100, ['halloweem bg0', 'halloweem bg lightning strike']);
				} else {
					halloweenBG = new BGSprite('halloween_bg_low', 0, -100);
				}
				add(halloweenBG);
			...
			case 'limo': //Week 4
				...
				if(!ClientPrefs.lowQuality) {
					limoMetalPole = new BGSprite('gore/metalPole', -300, 220, 0.4, 0.4);
					add(limoMetalPole);

					bgLimo = new BGSprite('limo/bgLimo', 50, 480, 0.4, 0.4, ['background limo pink'], true);
					add(bgLimo);

					limoCorpse = new BGSprite('gore/noooooo', -300, limoMetalPole.y - 130, 0.4, 0.4, ['Henchmen on rail'], true);
					add(limoCorpse);

					limoCorpseTwo = new BGSprite('gore/noooooo', -300, limoMetalPole.y, 0.4, 0.4, ['henchmen death'], true);
					add(limoCorpseTwo);
					...
					for (i in 0...5)
					{
						var dancer:BackgroundDancer = new BackgroundDancer((370 * i) + 330, bgLimo.y - 400);
						dancer.scrollFactor.set(0.4, 0.4);
						grpLimoDancers.add(dancer);
					}

					limoLight = new BGSprite('gore/coldHeartKiller', limoMetalPole.x + 20, limoMetalPole.y - 80, 0.4, 0.4);
					add(limoLight);
				...
				limo = new BGSprite('limo/limoDrive', 80, 550, 1, 1, ['Limo stage'], true);
			...
			case 'splatoon': //Week 7, all from previous 0.4 build
				defaultCamZoom = 0.76;
				...
				if(!ClientPrefs.lowQuality) {
					...
					//upperBoppers.setIdle('Upper Crowd Bob');			//#FIXME
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					upperBoppers.updateHitbox();
					add(upperBoppers);
				...
				var bgSnow:BGSprite = new BGSprite('week7/bgSnow', -600, 650);
			 add(bgSnow);
		...
		switch(curStage)
		{
			...
			case 'splatoon': //imported from version 0.4
				dad.x -= 180;	 //constructs the stage positions and ezili gun anims
				dad.y -= 40;	//otherwise the chart for octo-amazone is going to be really long
				boyfriend.x += 600;	//and require quite a bit of retooling the sprite sheets.
				gf.x += 160;
				gf.y -= 60;
				eziliGun = new Character(dad.x - 40, dad.y - 20, 'eziligun');
				eziliGun.antialiasing = true;
				eziliGun.alpha = 0;
				add(eziliGun);
				...
				eziliDance1 = new FlxSprite(dad.x - 162, dad.y - 682);
				eziliDance1.antialiasing = true;
				...
				eziliDance2 = new FlxSprite(dad.x - 80, dad.y - 750);
				eziliDance2.antialiasing = true;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			if (curStage == 'splatoon') { //imported from version 0.4
				if (!danc) {								//controls ezili's gun animations while idle
					if (eziliGunActive) {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		/*switch (curStage){
		 case 'splatoon':
			 comboSpr.setPosition(-500, 100);

		 case 'shed' | 'shed2':
			 comboSpr.setPosition(50, 50);

		 case 'alleyback':
			 comboSpr.setPosition(50, 50);

		 case 'limo':
			 comboSpr.setPosition(200, 50);

		 case 'spooky':
			 comboSpr.setPosition(100, 100);

		 default:
			 comboSpr.setPosition(500,0);
		 }*/
	...
	function eziliGunDance() { //imported from v0.4
		if (eziliGunShot) {			 //controls shooting animations
			eziliGunShot = false;
		} else {
			if (!eziliGunDanced)
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			...
			case 'splatoon': //imported from v0.4
				if(!ClientPrefs.lowQuality) {	//controls dancers for the splatoon stage
					upperBoppers.dance(true);
				}
